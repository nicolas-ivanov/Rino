...
public class FSMCommandAnalyzer extends AsyncTask<String, String, Intent> {
	...
	*/
	...
	private InputStream keywordsStream;
	private BufferedReader keywordsReader;
	private Lexemizer lexemizer;
	...
	public FSMCommandAnalyzer(MainActivity main) {//, InputStream stream) {
		...
		mainActivity = main;
		InputStream stream = main.getApplicationContext().getResources().
				openRawResource(R.raw.keywords_dict);
		keywordsStream = stream;		
		keywordsReader = new BufferedReader(new InputStreamReader(
				keywordsStream));
		stream = main.getApplicationContext().getResources().
				openRawResource(R.raw.simple_patterns);
		patternsStream = stream;
		patternsReader = new BufferedReader(new InputStreamReader(
				patternsStream));
		lexemizer = new Lexemizer(patternsReader);
		commandParser = new CommandsParser(keywordsReader);
	}
	...
	protected Intent doInBackground(String... commands) {
		...
		//try {
			...
			while (tokenizer.hasNextToken()){
				...
				String wordType = lexemizer.getLexemeType(word);
				Log.d(MainActivity.TAG, FSMCommandAnalyzer.TAG + "Word: " + word + " Word type: " + wordType);
				String[] keywordTypesRaw = commandParser.getCommandType(word);
				if (keywordTypesRaw != null) {
					Collection<String> keywordTypes = Arrays.asList(keywordTypesRaw);
			...
			while (!found && ((line = patternsReader.readLine()) != null)) {
				...
				if (structureMatcher.matches()) {
					...
					if (found) {
						...
						Log.d(MainActivity.TAG, "AsyncTask: pattern ID = '"
								+ patternID + "'");
						...
						if (patternID.equals("call_num")) {
							...
							if ((numberNum = findGroupNum("number",
									structureMatcher)) != null) {
							...
							} else if ((contactNum = findGroupNum("contact",
									structureMatcher)) != null) {
								...
								if (contacts.size() == 0) {
									...
									if (numbers == null) {
										...
										if (iterNumbers.hasNext()) {
											// TODO: take not only first phone
											// number
							...
							Integer contactNum;
							if ((contactNum = findGroupNum("contact",
									structureMatcher)) != null) {
								String name = commandMatcher.group(contactNum);
								Collection<Contact> contacts = ContactsDatabase
										.getInstance().getContacts(name);
							...
							if ((numberNum = findGroupNum("number",
								...
								Log.d(MainActivity.TAG,
										"AsyncTask: commandMatcher.group('number') = '"
												+ commandMatcher
														.group(numberNum) + "'");
							...
							Integer webpageNum;
							if ((webpageNum = findGroupNum("webpage",
									structureMatcher)) != null) {
								Log.d(MainActivity.TAG,
										"AsyncTask: commandMatcher.group('webpage') = '"
												+ commandMatcher
														.group(webpageNum)
												+ "'");
								...
								resIntent = new Intent(Intent.ACTION_VIEW,
										webpageUri);
								return resIntent;
							}
						}
							...
							if ((emailNum = findGroupNum("email",
									structureMatcher)) != null
									&& (textNum = findGroupNum("text",
											structureMatcher)) != null) {
								Log.d(MainActivity.TAG,
										"AsyncTask: commandMatcher.group('email') = '"
												+ commandMatcher
														.group(emailNum) + "'");
								Log.d(MainActivity.TAG,
										"AsyncTask: commandMatcher.group('text') = '"
								...
								resIntent = new Intent(Intent.ACTION_SENDTO,
										emailUri);
							...
							if ((hoursNum = findGroupNum("hours",
									structureMatcher)) != null) {

								Log.d(MainActivity.TAG,
										"AsyncTask: commandMatcher.group('hours') = '"
												+ commandMatcher
														.group(hoursNum) + "'");

								Integer hour = Integer.parseInt(commandMatcher
										.group(3));
								Integer minutes = 0;
								if ((minutesNum = findGroupNum("minutes",
										structureMatcher)) != null) {
									Log.d(MainActivity.TAG,
											"AsyncTask: commandMatcher.group('minutes') = '"
													+ commandMatcher
															.group(minutesNum)
													+ "'");
								...
								resIntent.putExtra(AlarmClock.EXTRA_MINUTES,
										minutes);
					...
					Log.d(MainActivity.TAG, "AsyncTask: Pattern '" + rawPattern
							+ "' is incorrect");
					return null;
				}
			} // while (!found && ((line = patternsReader.readLine()) != null))
